...
function LogForm({ title, value, onChange, onSave, onCancel, saveLabel = "💾 保存" }) {
  ...
  return (
    <div>
      ...
      <Field label="日付">
        <input type="date" value={value.date} onChange={e => onChange(deepClone({ ...value, date: e.target.value }))} style={inputStyle} />
      </Field>
      <Field label="記録の種類">
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {Object.entries(typeLabel).map(([k, v]) => (
            <button key={k} onClick={() => onChange(deepClone({ ...value, type: k }))}
      ...
      <Field label="天気">
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {WEATHER_OPTIONS.map(w => (
            <button key={w} onClick={() => onChange(deepClone({ ...value, weather: w }))}
      ...
      <Field label="気温 (℃)">
        <input type="number" value={value.temp} onChange={e => onChange(deepClone({ ...value, temp: e.target.value }))} placeholder="例: 22" style={inputStyle} />
      </Field>
      <Field label="水やり">
        <div onClick={() => onChange(deepClone({ ...value, watered: !value.watered }))}
      ...
      {value.type === "growth" && (
        <Field label="草丈 (cm)">
          <input type="number" value={value.height} onChange={e => onChange(deepClone({ ...value, height: e.target.value }))} placeholder="例: 25" style={inputStyle} />
        </Field>
      )}
      {value.type === "harvest" && (
        <Field label="収穫数（個）">
          <input type="number" value={value.harvest} onChange={e => onChange(deepClone({ ...value, harvest: e.target.value }))} placeholder="例: 5" style={inputStyle} />
        </Field>
      )}
      <Field label="メモ">
        <textarea value={value.note} onChange={e => onChange(deepClone({ ...value, note: e.target.value }))} placeholder="気づいたこと、変化など…" rows={3} style={{ ...inputStyle, resize: "vertical" }} />
      </Field>
      {/* 写真 */}
      <Field label="写真">
        <PhotoUploader photos={value.photos || []} onChange={photos => onChange(deepClone({ ...value, photos }))} />
      </Field>
...
export default function App() {
  ...
  function startEdit(log) {
    ...
    const logCopy = deepClone({ ...log, photos: log.photos || [] });
    setLogDraft(logCopy);
    setView("editLog");
  }
